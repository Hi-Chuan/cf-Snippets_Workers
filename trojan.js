...
let SUB = 'sub.841110.xyz';  // 自定义的订阅源
...
async function sha224(text){const data=new TextEncoder().encode(text);const K=[0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,0x923f82a4,0xab1c5ed5,0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,0xe49b69c1,0xefbe4786,0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,0x06ca6351,0x14292967,0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,0xa2bfe8a1,0xa81a664b,0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,0x5b9cca4f,0x682e6ff3,0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2];let H=[0xc1059ed8,0x367cd507,0x3070dd17,0xf70e5939,0xffc00b31,0x68581511,0x64f98fa7,0xbefa4fa4];const ml=data.length,bl=ml*8,pl=Math.ceil((ml+9)/64)*64,p=new Uint8Array(pl);p.set(data);p[ml]=0x80;new DataView(p.buffer).setUint32(pl-4,bl,false);for(let c=0;c<pl;c+=64){const W=new Uint32Array(64);for(let i=0;i<16;i++)W[i]=new DataView(p.buffer).getUint32(c+i*4,false);for(let i=16;i<64;i++){const s0=rightRotate(W[i-15],7)^rightRotate(W[i-15],18)^(W[i-15]>>>3),s1=rightRotate(W[i-2],17)^rightRotate(W[i-2],19)^(W[i-2]>>>10);W[i]=(W[i-16]+s0+W[i-7]+s1)>>>0;}let[a,b,c1,d,e,f,g,h]=H;for(let i=0;i<64;i++){const S1=rightRotate(e,6)^rightRotate(e,11)^rightRotate(e,25),ch=(e&f)^(~e&g),t1=(h+S1+ch+K[i]+W[i])>>>0,S0=rightRotate(a,2)^rightRotate(a,13)^rightRotate(a,22),maj=(a&b)^(a&c1)^(b&c1),t2=(S0+maj)>>>0;h=g;g=f;f=e;e=(d+t1)>>>0;d=c1;c1=b;b=a;a=(t1+t2)>>>0;}H[0]=(H[0]+a)>>>0;H[1]=(H[1]+b)>>>0;H[2]=(H[2]+c1)>>>0;H[3]=(H[3]+d)>>>0;H[4]=(H[4]+e)>>>0;H[5]=(H[5]+f)>>>0;H[6]=(H[6]+g)>>>0;H[7]=(H[7]+h)>>>0;}return H.slice(0,7).map(x=>(x>>>0).toString(16).padStart(8,'0')).join('');}
...
async function handleTroRequest(request, customProxyIP) {
  ...
  readable.pipeTo(new WritableStream({
    async write(chunk) {
      ...
      const trojanResult = await parsetroHeader(chunk, UUID);
  ...
  async fetch(request, env) {
    try {
      ...
      if (request.headers.get('Upgrade') === 'websocket') {
        ...
        if (url.pathname.toLowerCase() === `/${loginPwd.toLowerCase()}`) { 
        ...
        if (url.pathname.toLowerCase() === `/sub/${subPath.toLowerCase()}` || url.pathname.toLowerCase() === `/sub/${subPath.toLowerCase()}/`) {
          ...
          if (SUB) {
            try {
              let isApi = false;
              let up = SUB.trim().replace(/^https?:\/\//, "").replace(/\/$/, "");
              
              const p = new URLSearchParams();
              p.append('uuid', UUID);
              p.append("host", up);
              p.append("sni", up);
              p.append("path", `/?ed=2560`);
              p.append("type", "ws");
              p.append('security', 'tls');
              p.append('fp', 'firefox');
              
              const apiRes = await fetch(`https://${up}/sub?${p.toString()}`, {
              ...
              if (apiRes.ok) {
                ...
                try {
                  ...
                  decoded = decoded.replace(/host=[^&]*/g, `host=${currentDomain}`)
                           .replace(/sni=[^&#]*/g, `sni=${currentDomain}`)
                           .replace(/fp=[^&#]*/g, `fp=firefox`)
                           .replace(/vless:\/\//ig, `${troHeader}://`);
                  nodesText = decoded;
                  isApi = true;
                } catch (e) {
                  isApi = false; 
              ...
              if (!isApi) {
                const txtRes = await fetch(SUB.startsWith('http') ? SUB : `https://${SUB}`);
                if (txtRes.ok) {
                  const text = await txtRes.text();
                  const ips = text.split('\n').map(line => line.trim()).filter(line => line.length > 0 && !line.startsWith('#'));
                  if (ips.length > 0) {
                    const troLinks = ips.map(cdnItem => {
                      ...
                      return `${troHeader}://${UUID}@${host}:${port}?security=tls&sni=${currentDomain}&fp=firefox&allowInsecure=0&type=ws&host=${currentDomain}&path=%2F%3Fed%3D2560#${troNodeName}`;
                    });
                    nodesText = troLinks.join('\n');
                  }
                }
              }
            } catch (err) {
          ...
          const base64Content = btoa(unescape(encodeURIComponent(nodesText)));
