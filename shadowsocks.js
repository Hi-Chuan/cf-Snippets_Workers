                    ...
                    if (cfip.length === 1 && (cfip[0].startsWith('http://') || cfip[0].startsWith('https://'))) {
                        try {
                            ...
                            if (response.ok) {
                                ...
                                currentCfip = text.split('\n').map(line => line.trim()).filter(line => line.length > 0);
                            }
                        } catch (err) {
                            currentCfip = ['mfa.gov.ua#获取远程IP失败'];
                        }
                    }
                    const ssLinks = currentCfip.map(cdnItem => {
